//testing monad testnet
const tokenAddress = '0x26a1DF29a9aCde42042838f0cA5414a5688Ea03C';
const stakingAddress = '0xcc7178Cd216Ee3CFBA9726359AcA88Ee990ef78d';
...
async function switchToMonadChain() {
  if (typeof window.ethereum !== 'undefined') {
    try {
      ...
      if (currentChainId !== MONAD_CHAIN_ID) {
        ...
        console.log('Successfully moved to Monad');
      }
    } catch (switchError) {
      ...
      if (switchError.code === 4902) {
        try {
          ...
          console.log('Monad successfully added and selected');
        } catch (addError) {
...
window.addEventListener('load', switchToMonadChain);
	  ...
	  try {
		...
		if (currentChainId !== MONAD_CHAIN_ID) {
		  try {
			...
			alert("Successfully switched to Monad network!");
		  } catch (switchError) {
			if (switchError.code === 4902) {
			  await window.ethereum.request({
				method: 'wallet_addEthereumChain',
			  ...
			  alert("Monad Network successfully added and selected!");
			} else {
...
async function stakeOrApprove() {
	if (!userAccount) return alert("Wallet is not connected yet");
	  ...
	  try {
		await tokenContract.methods.approve(stakingAddress, web3.utils.toWei('1000000', 'ether'))
		  ...
		  .on('transactionHash', (hash) => {
			showTxNotification(hash, "Approve successful! Click to view TX");
		  });
	  } catch (err) {
...
async function stakeAllTokens() {
	...
	try {
	  await stakingContract.methods.stake(balance).send({ from: userAccount })
		.on('transactionHash', (hash) => {
		  showTxNotification(hash, "Stake all tokens successfully! Click to view TX");
		});
	} catch (error) {
	  alert("Stake failed");
	}
...
async function unstake() {
	if (!userAccount) return alert("Wallet belum terhubung");
...
async function unstakeAllTokens() {
	...
	await stakingContract.methods.unstake(stakedAmount).send({ from: userAccount })
	  .on('transactionHash', (hash) => {
		showTxNotification(hash, "Unstake all successful! Click to view TX");
	  });
  }
...
async function updateStakingInfo() {
  try {
      if (!userAccount) {
          console.log("The wallet address is not yet connected");
          return;
      }
...
function showTxNotification(txHash, message = "Transaction successful!") {
	...
	toast.onclick = () => window.open(`https://testnet.monadexplorer.com/tx/${txHash}`, '_blank');
